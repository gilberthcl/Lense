...
import type { AvailableModels, TenantModelInfo, TenantModelsList } from "../lib/types";
import { useToast } from "./Toast";
import { Badge, Button, Card, PanelHeader, Spinner } from "./ui";

/** Per-client base-model selector (W0b) — compliance-gated. */
function BaseModelSelect({ tid }: { tid: string }) {
  ...
  return (
    <div className="mb-4 rounded border border-slate-800 bg-slate-950/40 p-3">
      ...
      {!data.reachable && (
        <p className="mb-2 text-[11px] text-amber-300">Ollama unreachable — showing no installed models.</p>
      )}
      <select
      ...
      <p className="mt-1.5 text-[11px] text-slate-500">
        ...
        (below) overrides this base for analysis.
      </p>
    </div>
  );
}
